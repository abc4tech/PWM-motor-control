...
#define SENSOR_ON (GICR |= 0x40)  // povoli int0
#define SENSOR_OFF (GICR &= ~0x40)  // zakaze int0
...
/*
 * Output delay counter interupt service routine
 */
ISR(TIMER0_OVF_vect)
{
...
void motor_timer_start()
{
  TIMSK |= 0x01;        // enable overflow interupt from counter 1
